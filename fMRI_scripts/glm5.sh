...
MASK=individual_glm/mask_union.nii.gz
...
OUTPUTTAG=individual_glm/glm${GLM_IDX}_subject${ID}
...
if [ $CHECK_NORESP -eq 4 ]; then
  ...
  3dDeconvolve -input \
    ...
    -num_stimts 3 \
    -stim_label 1 RT -stim_times_AM2 1 $ST_RT 'UBLOCK(1)' \
    -stim_label 2 gain -stim_times_AM1 2 $ST_GAIN 'UBLOCK(1)' \
    -stim_label 3 loss -stim_times_AM1 3 $ST_LOSS 'UBLOCK(1)' \
    -allzero_OK \
    -GOFORIT 4 \
    -censor $FWD \
  ...
  3dDeconvolve -input \
    ...
    -stim_label 2 gain -stim_times_AM1 2 $ST_GAIN 'UBLOCK(1)' \
    -stim_label 3 loss -stim_times_AM1 3 $ST_LOSS 'UBLOCK(1)' \
    -stim_label 4 noResp -stim_times_IM 4 $ST_NORESP 'UBLOCK(1)' \
    -allzero_OK \
    -GOFORIT 4 \
    -censor $FWD \
    -bucket ${OUTPUTTAG}.nii.gz -tout -x1D $OUTPUTTAG
